...
const BASE_URL = 'https://www.kredibel.co.id/phone/id/';
...
if ('CANCEL' == @$RequestContentAsJson['data']['submit']){
  Output(0, "Cek Nomor Telepon telah dibatalkan.\nTerima kasih");
}

if ('OK' != @$RequestContentAsJson['data']['submit']){
  //Build quetions  
  $Text = "*Info Kredibel*";
  $Text .= "\nAnda ingin bertransaksi online?\nAda nomor mencurigakan atau tidak dikenal yang menghubungi?\n*Cek dulu disini!*";
  $Text .= "\nMohon lengkapi pertanyaan berikut.";
  $Text .= "\n";

  $generalQuestion[] = AddQuestion('string', 'phone', "Masukkan Nomor Telepon/Ponse/WA yang dicurigai");

  $questionData[] = $generalQuestion;

  $url = GetBaseUrl() . '/services/other/kredibel/phone/';
...
$buttons[] = AddButtonURL("❗️ Laporkan Nomor", $url);
$buttonList[] = $buttons;
